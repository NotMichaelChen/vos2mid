...
struct note_t
{
    uint begin; /* note hit time */
    uint end; /* note release time */
    union
    {
        ubyte[4] bytes;
        uint data;
...
smf_t* vos2smf(vos_t* vos)
{
    ...
    for (retval.ntracks = vos.smf.ntracks; retval.ntracks < nchannel * 2 + vos.smf.ntracks;
            retval.ntracks++)
    {
        retval.tracks[retval.ntracks].nevents = 0;
        ...
        {
            retval.tracks[retval.ntracks].events = cast(event_t * ) malloc(event_t.sizeof * nevent);
            if (!retval.tracks[retval.ntracks].events)
                return retval;
        }
    }
    retval.division = vos.smf.division;
    retval.ttempo = vos.smf.ttempo;
    retval.format = vos.smf.format;
    ...
    for (ntrack = 0; ntrack < nchannel; ntrack++)
    {
        ...
        for (nevent = 0; nevent < vos.channels[ntrack].nnotes; nevent++)
        {
            ...
            /* ontrk.events[nevent].bytes[3]=0; */
            offtrk.events[nevent].tick = note.end * retval.division / 768;
            offtrk.events[nevent].data = note.data;
            offtrk.events[nevent].bytes[2] = 0;
            /* offtrk.events[nevent].bytes[3]=0; */
        }
        { /* insertion sort */
            size_t i;
            event_t event;
...
vos_t* vos_parser(ubyte* chunk, size_t size)
{
    ubyte[] vos_header = [
        0x3, 0x0, 0x0, 0x0, 0x40, 0x0, 0x0, 0x0, 0x69, 0x6e, 0x66, 0x0
    ];
    size_t offset = 0;
    uint sub_chuck; /* subchunk info */
    uint chunk_length; /* subchunk size */
    ushort channel; /* current channel number, start from 0 */
    vos_t* retval = null; /* return value */
    if (vos_header.sizeof > size)
        return retval; /* buffer overflow */
    if (!memcmp(chunk, &vos_header[0], vos_header.sizeof)) /* vos file */
    {
        offset = 0x18;
        ...
        for (channel = 0; channel < retval.nchannels; channel++)
        {
            ...
            if (nnotes)
            {
                ...
                retval.channels[channel].notes = cast(note_t * ) malloc(note_t.sizeof * nnotes);
                if (!retval.channels[channel].notes)
                    return retval;
                ...
                for (nnotes = 0; nnotes < retval.channels[channel].nnotes; nnotes++)
                {
                    ...
                    retval.channels[channel].notes[nnotes].end
                        += retval.channels[channel].notes[nnotes].begin;
                    offset += 4;
                    /* midi event */
                    retval.channels[channel].notes[nnotes].bytes[0] = chunk[offset++];
                    retval.channels[channel].notes[nnotes].bytes[1] = chunk[offset++];
                    retval.channels[channel].notes[nnotes].bytes[2] = chunk[offset++];
                    /* reserved byte */
                    retval.channels[channel].notes[nnotes].bytes[3] = chunk[offset + 1] ? 0x80 : 0x00;
                    if (chunk[offset] & 0x80)
                        retval.channels[channel].notes[nnotes].bytes[3] |= (
                                (chunk[offset] & 0x70) >> 4) + 1;
                    offset += 2;
                }
            }
        }
                    ...
                    && chunk[offset + 2] == 0x53 && chunk[offset + 3] == 0x30) /* "VOS0" */
            {
                ...
                for (channel = 0; channel < retval.nchannels; channel++)
                {
                    ...
                    if (nnotes)
                    {
                        ...
                        if (!retval.channels[channel].notes)
                            return retval;
                        retval.channels[channel].nnotes = nnotes;
                        for (nnotes = 0; nnotes < retval.channels[channel].nnotes;
                                nnotes++)
                        {
                            offset++;
                            retval.channels[channel].notes[nnotes].begin = chunk[offset + 3];
                            ...
                            offset += 4;
                            retval.channels[channel].notes[nnotes].bytes[1] = chunk[offset++];
                            retval.channels[channel].notes[nnotes].bytes[0] = chunk[offset++] | 0x90;
                            retval.channels[channel].notes[nnotes].bytes[2] = chunk[offset++];
                            offset++;
                            offset++;
                            retval.channels[channel].notes[nnotes].bytes[3] = chunk[offset++]
                                ? 0x80 : 0x00;
                            retval.channels[channel].notes[nnotes].end = chunk[offset + 3];
                            retval.channels[channel].notes[nnotes].end <<= 8;
                            retval.channels[channel].notes[nnotes].end += chunk[offset + 2];
                ...
                offset += 4; /* skip 4 magic bytes */
                {
                    ...
                    while (nnotes)
                    {
                        ...
                        if (chunk[offset] < retval.nchannels
                                && index < retval.channels[chunk[offset]].nnotes)
                            retval.channels[chunk[offset]].notes[index].bytes[3] |= (
                                    chunk[offset + 5] & 0x0f) + 1;
                        else
                            {} /* ignore illegal key */
                        nnotes--;
                        offset += 6;
